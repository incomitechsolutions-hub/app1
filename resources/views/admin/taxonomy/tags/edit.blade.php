...
@section('title', 'Taxonomy Skill bearbeiten')
@section('breadcrumb', 'Kategorien')

@section('content')
    <div class="mx-auto max-w-xl space-y-6">
        <div class="flex items-center justify-between gap-4">
            <h1 class="text-3xl font-bold text-slate-900">Taxonomy Skill bearbeiten</h1>
            <a href="{{ route('admin.taxonomy.tags.index') }}"
                class="text-sm font-medium text-slate-600 hover:text-slate-900">Zur Übersicht</a>
        </div>

        <form method="post" action="{{ route('admin.taxonomy.tags.update', $tag) }}" class="admin-panel space-y-4 p-6">
            @csrf
            @method('PUT')
            <div>
                <label for="name" class="block text-sm font-medium text-slate-700">Name</label>
                <input id="name" name="name" type="text" required value="{{ old('name', $tag->name) }}"
            ...
            <div>
                ...
                <input id="slug" name="slug" type="text" required value="{{ old('slug', $tag->slug) }}"
                    class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm shadow-sm">
                @error('slug')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
