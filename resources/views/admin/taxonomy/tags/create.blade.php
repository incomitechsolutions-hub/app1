...
@section('title', 'Taxonomy Skill anlegen')
@section('breadcrumb', 'Kategorien')

@section('content')
    <div class="mx-auto max-w-xl space-y-6">
        <div class="flex items-center justify-between gap-4">
            <h1 class="text-3xl font-bold text-slate-900">Taxonomy Skill anlegen</h1>
            <a href="{{ route('admin.taxonomy.tags.index') }}"
                class="text-sm font-medium text-slate-600 hover:text-slate-900">Zur Übersicht</a>
        </div>

        <form method="post" action="{{ route('admin.taxonomy.tags.store') }}" class="admin-panel space-y-4 p-6">
            @csrf
            <div>
            ...
            <div>
                ...
                <input id="slug" name="slug" type="text" value="{{ old('slug') }}" placeholder="wird aus Name erzeugt"
                    class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm shadow-sm">
                @error('slug')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
