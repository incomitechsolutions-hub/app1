...
@section('title', 'Markt anlegen')
@section('breadcrumb', 'Länder')

@section('content')
    <div class="mx-auto max-w-2xl space-y-6">
        <h1 class="text-3xl font-bold text-slate-900">Markt anlegen</h1>
        <form method="post" action="{{ route('admin.localization.markets.store') }}" class="admin-panel space-y-4 p-6">
            @csrf
            <div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="display_code" value="{{ old('display_code') }}" required maxlength="8" class="mt-1 w-full rounded-lg border-slate-300 text-sm" placeholder="DE">
                ...
                <div>
                    ...
                    <input type="text" name="country_code" value="{{ old('country_code') }}" maxlength="2" class="mt-1 w-full rounded-lg border-slate-300 text-sm uppercase" placeholder="DE">
                    @error('country_code')<p class="mt-1 text-xs text-rose-600">{{ $message }}</p>@enderror
                </div>
            </div>
            <div>
                <label class="block text-sm font-medium text-slate-700">Domain (Host)</label>
                <input type="text" name="domain" value="{{ old('domain') }}" required class="mt-1 w-full rounded-lg border-slate-300 text-sm" placeholder="example.de">
            ...
            <div>
                ...
                <input type="number" name="vat_rate" value="{{ old('vat_rate', '19') }}" step="0.01" min="0" max="100" required class="mt-1 w-full rounded-lg border-slate-300 text-sm">
            ...
            <div>
                ...
                <select name="default_locale_id" class="mt-1 w-full rounded-lg border-slate-300 text-sm">
                    ...
                    @foreach ($locales as $loc)
                        <option value="{{ $loc->id }}" @selected(old('default_locale_id') == $loc->id)>{{ $loc->name }} ({{ $loc->code }})</option>
                    @endforeach
                </select>
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('admin.localization.markets.index') }}" class="rounded-lg border border-slate-300 px-4 py-2 text-sm text-slate-700 hover:bg-slate-50">Abbrechen</a>
            </div>
        </form>
    </div>
@endsection
