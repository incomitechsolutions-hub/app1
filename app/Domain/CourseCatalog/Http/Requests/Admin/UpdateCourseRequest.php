...
class UpdateCourseRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'external_course_code' => [
                ...
                'string',
                'max:64',
                Rule::unique('courses', 'external_course_code')->ignore($courseId),
            ],
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator): void {
            ...
            $course = $this->route('course');
            $courseId = $course instanceof Course ? (int) $course->getKey() : null;
            ...
            $relations = $this->input('course_relations', []);
            if (is_array($relations) && $courseId !== null) {
                foreach ($relations as $i => $row) {
                    if (! is_array($row)) {
                        continue;
                    }
                    $rid = (int) ($row['related_course_id'] ?? 0);
                    if ($rid > 0 && $rid === $courseId) {
                        $validator->errors()->add(
                            "course_relations.{$i}.related_course_id",
                            __('A course cannot relate to itself.')
                        );
                    }
                }
            }
